...
class Detector():
    ...
    def resize_GAN(self, img_path, img_name, is_video=False):
        ...
        if is_video is False:
            ...
            if granularity < 10:  #TODO: implement adaptive granularity by weighted changes
                print("Granularity of image was less than threshold at ",
                      granularity)
                granularity = 10
            # Resize image down
            try:
                mini_img = resize(image, (int(image.shape[1] / granularity),
            ...
            while success:
                ...
                if success:
                    ...
                    mini_img = resize(
                        image, (int(image.shape[1] / granularity),
                                int(image.shape[0] / granularity)),
    ...
    def run_ESRGAN(self, in_path=None, is_video=False, force_jpg=True):
        ...
        self.esrgan_instance = esrgan.test.esrgan(
            model_path=self.esr_model_path, hw=self.hardware)
        for img_path, img_name in img_list:
            self.resize_GAN(img_path=img_path,
                            img_name=img_name,
                            is_video=is_video)
        ...
        self.load_weights()
        for img_path, img_name in img_list:
            self.ESRGAN(img_path=img_path,
                        img_name=img_name,
                        is_video=is_video)
        fin = time.perf_counter()
        total_time = fin - star
        print(
            "Completed ESRGAN detection and decensor in {:.4f} seconds".format(
                total_time))
    ...
    def run_on_folder(self,
        ...
        self.esrgan_instance = [
        ]  # rare case where esrgan instance not destroyed but new action started, catch it here
        self.load_weights()
        if dilation < 0:
